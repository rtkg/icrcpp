...
namespace ICR
{
  ...
  //--------------------------------------------------------------------
  bool OWS::writeToFile(const std::string& path)const
  {
    remove(path.c_str());
    FILE* wn=fopen (path.c_str(),"a");
    if(!wn)
      {
	std::cout<<"Warning in OWS::writeToFile(const std::string& path)const Couldn't write to file"<<std::endl;
	return false;
      }
