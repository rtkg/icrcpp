...
namespace ICR
{
  ...
  void ObjectLoader::loadObject(std::string const& file,std::string const& name)
  {
    ...
    // if(!obj_parser.parse(file))
    //   {
    //     std::cout<<"Error in ObjectLoader::loadObject(std::string const& file,std::string const& name) - Couldn't parse file. Exiting..." <<std::endl;
    //     exit(0);
    //   }

    obj_parser.parse(file); //Returns always false ....?

    if(points_buffer.size() < normals_buffer.size())
      {
	std::cout<<"Error in ObjectLoader::loadObject(std::string const& file,std::string const& name) - Provide an .obj file with non-smoothed vertex normals. The mapping from vertices to vertex normals has to be unique. Exiting..." <<std::endl;
	exit(0);
      }
