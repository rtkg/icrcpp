...
void computeInitialSolution(DMatrix& initial_solution, const std::vector<std::vector<Eigen::MatrixXd* > >& Wh_i_list)
{
  ...
  for (uint i=0; i<Ih*L; i++)
   ...
   h_start_c_ind+=nCh;
      ...
      //================OBJECTIVE========================================
   f=f-x(deh_ind)-KAPPA*mul(x(zh_i_ind).trans(),SXMatrix(Ih,1,1));
 ...
 DMatrix ah(K,1);
      for (int k=0; k<K;k++)
	{
      ...
      g(tc_ind) = mul(x(eh_ind),SXMatrix(T,1,-1));
  ...
  g(sc_ind[i])=mul(SXMatrix(ah.trans()), mul(Whi,x(lmbdh_i_ind[i]))) +x(eh_ind)+x(deh_ind)+x(zh_i_ind[i]);
	  ub_g(sc_ind[i])=DMatrix(1,1,0);
	}
    }		  
  ...
  for (uint h=0; h<H; h++)
      ...
      for (int k=0; k<K;k++)
	{
      ...
      initial_solution=vertcat(initial_solution, ah/solution(deh_ind)); //nh'=nh/de
      initial_solution=vertcat(initial_solution, solution(eh_ind)/solution(deh_ind)+1); //Close Fit: e'=e/de+1
